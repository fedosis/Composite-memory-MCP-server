...
> **Version:** 1.0  
> **Applies to:** CMMS v0.5 (MCP stdio, single-threaded async, SQLite + Qdrant in-memory + Python dict graph)  
...
``` |
| **Interpretation** | Low MRR means the embedding model can't distinguish facts well, OR the auto-indexing pipeline isn't populating Qdrant correctly. First verify auto-indexing (see §1.7), then inspect embedding quality directly. |
...
``` |
| **Interpretation** | Shortest path not returned → BFS implementation bug or graph construction issue. Check `SimpleGraph.find_path()` depth parameter. If shortest path has more edges than expected, verify the direct edge was created during auto-indexing. |
...
| **How** | (a) Query with a known keyword from default rules (e.g., `"what is the IP of server"`) → verify `stage=1`. (b) Query with unknown text that has a semantically similar fact in Qdrant → verify `stage=2`. (c) Query with entity name from graph → verify `stage=3`. (d) Query with gibberish / no matches → verify `stage=4` (LLM fallback). |
...
async def bench_route_fallthrough():
    ...
    return results
``` |
| **Interpretation** | Wrong stage assignment → hybrid router logic defect. Rules stage misclassification is most common (keyword matching is case-sensitive, or rules not loaded). Stage 2→3 misclassification happens when embeddings produce no results but graph has a match — verify Qdrant is populated. |
...
### 1.7 Auto-Indexing — Fact Appears in Qdrant + Graph After `remember()`

| Property | Detail |
|----------|--------|
| **What** | After `remember()`, the fact is automatically upserted into Qdrant (vector search) and synced to the graph (entity nodes + edge). Verify both side effects occur. |
| **Why** | Auto-indexing is the bridge that keeps each storage backend in sync. If it silently fails (the `try/except` in `_auto_index_fact` catches all exceptions), later `semantic_search()` and `graph_search()` return empty results even though data was "remembered". |
| **How** | Call `remember()`, then query Qdrant directly and the graph directly for the fact. Qdrant: use the `_qdrant.search()` method with the fact's embedding. Graph: use `_graph.get_node()` for subject/object nodes and `_graph.get_edge()` for the predicate relation. |
| **Expected baseline** | 100% of remembered facts appear in both Qdrant and the graph within the same async tick. |
| **Threshold** | 100% |
| **Python pattern** | ```python
async def bench_auto_index():
    from memory_server.server import _get_qdrant_and_embedder, _get_graph_router, _get_provider
    ...
    vector = embedder.embed("test-subj test-pred test-obj")
    qdrant_res = await qdrant.search("memories", vector, limit=5)
    in_qdrant = any(p.id == fact_id for p in qdrant_res)
    ...
    return {"fact_id": fact_id, "in_qdrant": in_qdrant, "in_graph": in_graph}
``` |
| **Interpretation** | If a fact is missing from Qdrant but present in the graph (or vice versa), the `_auto_index_fact()` failed partially — the try/except caught an exception from one provider. Check logs for `"Auto-indexing failed"` warnings. If both are missing, the function was never called (check `remember_tool` wiring). |
...
| Property | Detail |
|----------|--------|
| **What** | End-to-end latency for storing a single fact via `remember()`, including auto-indexing (embedding + Qdrant upsert + graph sync) |
| **Why** | `remember()` is the synchronous write path — high latency blocks the MCP event loop (stdio, single-threaded) |
...
| **Expected baseline** | With in-memory Qdrant and MockEmbeddingProvider: p50 < 50ms, p95 < 100ms, p99 < 200ms. With real SentenceTransformer (all-MiniLM-L6-v2, CPU): p50 < 200ms, p95 < 500ms, p99 < 1000ms. |
...
``` |
| **Interpretation** | High p50 (≥1s with real model) suggests the embedding call is the bottleneck — it runs synchronously in `asyncio.to_thread()`. High p95/p99 spread suggests GC pauses or Qdrant rebalancing in memory. Compare mock vs real model to isolate embedding overhead. |
...
| **Why** | `search()` uses SQL `LIKE` across three text columns; without FTS5, performance degrades with volume |
...
| **Threshold** | p50 < 100ms at N=5000 for exact match; if >500ms, add FTS5 index |
| **Python pattern** | ```python
...
``` |
| **Interpretation** | Full-scan latency growing linearly with N is expected for `LIKE '%keyword%'` — SQLite has no index for that pattern. If it becomes a problem, switch to FTS5. Exact-match latency should be near-constant (fast). A spike indicates the SQL query plan changed or the table has too many columns. |
...
| **How** | Generate synthetic text with N `"X is Y"` sentences (N = {10, 100, 500}). Call `learn()` on the full text. Measure total wall-clock time. Compute throughput = N / seconds. Include auto-indexing time (the `learn_tool` function indexes each extracted fact to Qdrant + graph). |
...
``` |
| **Interpretation** | Low throughput (< 20 facts/sec regex) is likely caused by per-fact auto-indexing overhead: each extracted fact triggers an embedding call + Qdrant upsert + graph sync. Batch-embedding (via `embedder.embed_batch()` in `_auto_index_fact`) could mitigate this — but currently each fact is indexed individually. The `learn_tool` iterates over extracted facts sequentially. If throughput is critical, batch Qdrant upsert should be added. |
...
| Property | Detail |
|----------|--------|
| **What** | Resident memory (RSS) of the server process under three conditions: (a) baseline (server started, no facts stored), (b) 1000 indexed facts (Qdrant + graph populated), (c) embedding model loaded (after first `semantic_search`) |
...
| **Expected baseline** | Baseline (empty, no model): ~30–60 MB (Python interpreter + aiosqlite + qdrant-client). After 1000 facts: ~80–150 MB (Qdrant in-memory collection + graph nodes/edges + Python objects). After model load: ~200–400 MB (all-MiniLM-L6-v2 is ~90 MB model weights, but PyTorch + CUDA/cpu adds overhead). |
| **Threshold** | < 500 MB total under all conditions. If > 500 MB, investigate memory growth in Qdrant in-memory or unreleased graph references. |
| **Python pattern** | ```python
import psutil, os
...
``` |
| **Interpretation** | Large baseline (> 100 MB) → investigate imports (sentence-transformers may be imported eagerly). Large growth after 1000 facts (> 200 MB) → Qdrant in-memory may be using excessive storage per vector. Large growth after model load (> 500 MB) → PyTorch CUDA memory allocator may be pre-allocating (set `PYTORCH_CUDA_ALLOC_CONF=max_split_size_mb:128`). |
...
| **How** | Measure time from `python -m memory_server` to `ping()` returning `{"status": "ok"}`. Cold start: first launch (no model cached, no SQLite DB). Warm start: relaunch against the same in-memory state (Qdrant and graph must be rebuilt). |
...
### 3.2 Error Recovery — Qdrant Unavailable

| Property | Detail |
|----------|--------|
| **What** | When Qdrant is unavailable (e.g., in-memory initialization fails), the `_auto_index_fact()` function catches the exception and logs a warning. The `remember()` / `learn()` call still succeeds — only auto-indexing is skipped. |
| **Why** | Qdrant is a secondary index; the primary fact store is SQLite. Losing Qdrant should not lose data. |
| **How** | Mock a QdrantProvider that raises on every call, or set `location` to a nonexistent host. Call `remember()`. Verify the fact is stored in SQLite (via `search()`), but `semantic_search()` returns no results for that fact. |
| **Expected baseline** | `remember()` returns success with receipt + fact. `search()` finds the fact. `semantic_search()` does not find it. Warnings are logged. |
| **Threshold** | No crash, no data loss. |
| **Python pattern** | ```python
async def test_qdrant_unavailable():
    # Temporarily override qdrant to an unhealthy state
    from memory_server.server import _qdrant, _get_qdrant_and_embedder
    old_qdrant = _qdrant
    _qdrant = None  # Simulate init failure
    ...
    try:
        ...
        _qdrant = old_qdrant
``` |
| **Interpretation** | If `remember()` fails when Qdrant is down, the try/except in `_auto_index_fact` is not catching the exception properly → bug. If SQLite storage also fails, the providers share a connection pool that shouldn't be shared. |
...
| Property | Detail |
|----------|--------|
| **What** | After each tool call, all provider connections (aiosqlite connection, Qdrant client) should be properly reference-counted or closed. No file descriptors should accumulate. |
| **Why** | Long-running server with open FDs eventually hits the ulimit (typically 1024), causing `EMFILE` errors. |
| **How** | Before and after a burst of 1000 tool calls, check open file descriptors via `proc_fd_count = len(os.listdir('/proc/self/fd'))`. Also check SQLite connection count and Qdrant client count if accessible. |
| **Expected baseline** | FD count difference of 0–2 (minor GC-related fluctuation). SQLite uses `aiosqlite` which manages a single connection; Qdrant in-memory uses gRPC-in-process (no external FDs). |
...
``` |
| **Interpretation** | A large delta (> 10) suggests unclosed file handles. Common culprits: `QdrantClient` re-initialization in `_get_qdrant_and_embedder()` (each call re-creates the client), or `asyncio.to_thread` threads that hold references. Because providers are singletons (lazy-init once), this should be stable. |
...
| **Why** | The graph engine (`SimpleGraph`) adds nodes during auto-indexing but has no corresponding deletion path — `remember()` has no `delete()` tool, and the graph nodes persist even if the SQLite fact is removed. |
...
``` |
| **Interpretation** | Each `remember()` creates 2 graph nodes (subject, object) + 1 edge. Deleting the SQLite fact does NOT remove graph nodes. Expected orphan count = 2 × cycles (2000 at 1000 cycles). If the count is higher, nodes are accumulating from a secondary source (e.g., `learn()` auto-indexing decisions/skills to graph). |
...
| **Why** | Long-lived stdio processes must remain stable across hours of operation. Memory leaks, thread leaks, or Qdrant collection bloat only manifest over time. |
...
| **Expected baseline** | 0 crashes. Memory growth < 50 MB over the hour. Latency p95 does not increase by more than 20% from start to end. No more than 1 transient error (network blip, OOM from Qdrant). |
...
``` |
| **Interpretation** | **Memory growth**: Qdrant in-memory collection grows with every `remember()` call — each fact adds a 384-dimensional vector + payload. Expected growth ~1–2 KB per fact; 360 facts over 1h ≈ 360–720 KB, negligible. If RSS grows > 50 MB, check for Python object accumulation in Validator, DecayEngine, or SimpleGraph stores. **Latency creep**: Embedding model may trigger JIT compilation or cache thrashing. **FD growth**: Qdrant gRPC channel leak is the primary risk. |
...
1. **Python 3.12+** with `pip install -e ".[dev,sentence]"`
2. **Clean state**: no pre-existing in-memory SQLite, Qdrant, or graph data
...
| Memory | ≥ 2 GB RAM (for model load + Qdrant in-memory) |
