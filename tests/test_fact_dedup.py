...
def test_fact_key_uses_shared_normalizer_and_unit_separator():
    ...
    assert fact_dedup_key("Docker  is", "x", "y") == fact_dedup_key("Docker is", "x", "y")
    assert fact_dedup_key("Docker  is", "x", "y") == fact_dedup_key("Docker is", "x", "y")
...
async def test_seeded_outer_and_internal_whitespace_reinforces(provider):
    fact = await provider.create_fact(make_fact(subject=" Docker  is ", object=" container "))
    async with provider._session_factory() as session:
        found = await FactRepository(session).find_existing("Docker is", "is", "container")
